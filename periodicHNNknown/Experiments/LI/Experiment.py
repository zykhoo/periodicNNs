...
def lossfuc(model,mat,x,y,device,x0,H0,dim,c1=1,c2=1,c3=1,c4=1,epoch=0,periods=None,verbose=False):
    ...
    for i in range(dim):
      if (periods[i]>0) and (i<int(dim/2)):
        zeta = torch.zeros(x.shape).to(device)
        zeta[:,i] = periods[i]
        zeta_plus = Variable((x.clone()+zeta).to(torch.float64),requires_grad=True).to(device)
        zeta_minus = Variable((x.clone()-zeta).to(torch.float64),requires_grad=True).to(device)
        y_plus = model(zeta_plus)
        y_minus = model(zeta_minus)
        dH_plus=torch.autograd.grad(y_plus, zeta_plus, grad_outputs=y_plus.data.new(y_plus.shape).fill_(1),retain_graph=True,create_graph=True, allow_unused=True)[0]
        dH_minus=torch.autograd.grad(y_minus, zeta_minus, grad_outputs=y_minus.data.new(y_minus.shape).fill_(1),retain_graph=True,create_graph=True, allow_unused=True)[0]
        dHdq_plus=dH_plus[:,i]
        dHdq_minus=dH_minus[:,i]
        f4+=torch.mean((dHdq_plus+mat[:,int(3*dim/2)+i])**2,dim=0)
        f4+=torch.mean((dHdq_minus+mat[:,int(3*dim/2)+i])**2,dim=0)
      elif (periods[i]>0) and (i>=int(dim/2)):
        zeta = torch.zeros(x.shape).to(device)
        zeta[:,i] = periods[i]
        zeta_plus = Variable((x.clone()+zeta).to(torch.float64),requires_grad=True).to(device)
        zeta_minus = Variable((x.clone()-zeta).to(torch.float64),requires_grad=True).to(device)
...
def get_loss(model,device,initial_conditions,bs,x0,H0,dim,wholemat,evalmat,c1,c2,c3,c4,epoch,periods,trainset=False,verbose=False):
    ...
    for count in range(0,len(mat),bs):
      ...
      loss,f1,f2,f3,f4=lossfuc(model,curmat,x,y,device,x0,H0,dim,c1,c2,c3,c4,epoch,periods,False)
...
def train(net,name,bs,num_epoch,initial_conditions,device,wholemat,evalmat,x0,H0,dim,LR,patience,c1,c2,c3,c4,periods):
    ...
    for epoch in range(num_epoch):        
        ...
        for count in range(0,len(wholemat),bs):
            ...
            loss,f1,f2,f3,f4=lossfuc(net,mat,x,y,device,x0,H0,dim,c1,c2,c3,c4,epoch,periods,False,)  
            loss.backward()
            if epoch < 4000: 
    ...
    return net,epoch,avg_vallosses,avg_lossli,avg_f1li,avg_f2li,avg_f3li,avg_f4li,periods
...
for i in range(5):
  ...
  for ini in initialcon: 
    ...
    net.load_state_dict(torch.load('%s/%s/%s_%s_%s_%s_%s.pt' %(str(Path(loc).parents[0])+ '/Baseline',s,"PINN",seed,ini,0,0)))
    snake3 = SnakeNet(sys.netshape1,sys.netshape2,1,periods)
    snake3 = weights_copy(snake3,net)
    starttime = time.time() 
    print("training PINN Net")
    torch.save(snake3.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"sinsq",seed,ini,0,0))
    results = train(snake3,name="%s/%s/%s_%s_sinsq" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=1,periods = sys.periods)
    snake3, epochs = results[0], results[1]
    PINNtraintime = time.time()-starttime
    torch.save(snake3.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"sinsq",seed,ini,epochs,PINNtraintime))
