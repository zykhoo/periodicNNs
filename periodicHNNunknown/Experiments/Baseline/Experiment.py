...
def cos_act(x, param1):
    return x + (param1)*(torch.cos(param1*x))*(torch.cos(param1*x))

class SnakeNet3(nn.Module):
    ...
    def forward(self, x):
        x = cos_act(self.hidden_layer_1(x), self.alpha)
        x = cos_act(self.hidden_layer_2(x), self.alpha) 
        x = self.output_layer(x)
