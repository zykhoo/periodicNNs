    ...
    for epoch in range(num_epoch):
        ...
        if epoch % 500 == 0 : 
            ...
            print('epoch=',epoch, ' time=', elapsed_time,
                  ...
                  ' f3=', avg_f3 ,' f4=', avg_f4 , 'num_batches=', num_batches, 'alpha=', net.alpha, 'percent lr=', optimizer.param_groups[0]["lr"] )
...
for i in range(5):
  ...
  for ini in initialcon: 
    ...
    snake3 = SnakeNet(sys.netshape1,sys.netshape2,1)
    ...
    results = train(snake3,name="%s/%s/%s_%s_sinsq" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0)
    ...
    snake = SnakeNet3(sys.netshape1,sys.netshape2,1)
    starttime = time.time() 
    print("training PINN Net")
    torch.save(snake.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"snake3",seed,ini,0,0))
    results = train(snake,name="%s/%s/%s_%s_snake3" %(loc,s,seed,ini),bs=int(len(wholemat)/5),num_epoch=150001,initial_conditions=initialcon,device=device, wholemat=wholemat,evalmat=evalmat,x0=x0,H0=H0,dim=dim,LR=LR,patience=4000,c1=1,c2=1,c3=1,c4=0)
    snake, epochs = results[0], results[1]
    PINNtraintime = time.time()-starttime
    torch.save(snake.state_dict(), '%s/%s/%s_%s_%s_%s_%s.pt' %(loc,s,"snake3",seed,ini,epochs,PINNtraintime))
