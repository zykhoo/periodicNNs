...
class IndLearnNet(nn.Module):

    def __init__(self, input_size, hidden_size, output_size):
        super(IndLearnNet, self).__init__()
        self.hidden_layer_1 = nn.Linear( input_size, hidden_size, bias=True)
        self.hidden_layer_2 = nn.Linear( hidden_size, hidden_size, bias=True)
        self.output_layer = nn.Linear( hidden_size, output_size, bias=True)
        self.alpha = nn.Parameter(torch.ones(1))
...
for s in ["pendulum", "System3", "System5", "System11", "System12","doublepend"]: #["pendulum","trigo","arctan","System2","System3","System4","System5","System6","System7","System8","System9","System10","System11","System12","System13","System14"]: #["pendulum","trigo","arctan","System2","System3","System4","System5","System6","System7","System8","System9","System10","System11","System12","System13","System14"]:
    ...
    z = torch.tensor(np.array(np.meshgrid(*[np.linspace(sys.spacedim[i][0]-sys.periods[i], sys.spacedim[i][1]+sys.periods[i], 20) if sys.periods[i]>0 else np.linspace(sys.spacedim[i][0], sys.spacedim[i][1], 10) for i in range(dim) ],
                                      )))
    ...
    for f in glob.glob(loc+"/Experiments/Baseline/%s/*.pt" %s):
        ...
        if (len(details) == 5) & (details[0] == "PINN"):
            ...
            if  (int(details[3])>0):
              if math.isnan(data["Hloss"]) or math.isinf(data["Hloss"]):
                data["Hloss"] = 10000
                data['highlight']=0.5
              if math.isnan(data["loss"]) or math.isinf(data["loss"]):
                data["loss"] = 10
                data['highlight']=0.5
              df = pd.concat([df,data.to_frame().T], ignore_index = True)

    for f in glob.glob(loc+"/Experiments/LearningPeriodParameter/%s/*.pt" %s):
        details = f.split("/")[-1].split("_")
        fvec = lambda z: np.concatenate([np.expand_dims(sys.f1(z),0), np.expand_dims(sys.f2(z),0)]) if len(sys.spacedim)==2 else np.concatenate([sys.f1(z), sys.f2(z)])
    ...
    for f in glob.glob(loc+"/Experiments/InductiveSnakeLiu/%s/*.pt" %s):
        ...
        if (len(details) == 5) & (details[0][:5] == "sinsq"):
                periods = np.asarray(sys.periods)
                periods = np.min(periods[np.nonzero(periods)])/(np.pi)
                net = SnakeNet(sys.netshape1,sys.netshape2,1)
                net.load_state_dict(torch.load(f))
                model = "pHNN-I"
                net = net.to(device)
                data = pd.Series({'seed':int(details[1]), 
    ...
    for f in glob.glob(loc+"/Experiments/LI/%s/*.pt" %s):
        details = f.split("/")[-1].split("_")
        fvec = lambda z: np.concatenate([np.expand_dims(sys.f1(z),0), np.expand_dims(sys.f2(z),0)]) if len(sys.spacedim)==2 else np.concatenate([sys.f1(z), sys.f2(z)])
        if (len(details) == 7) & (details[0][:5] == "PINNL"):
                net = IndLearnNet(sys.netshape1,sys.netshape2,1)
                model = "pHNN-LI"
                net = net.to(device)
                data = pd.Series({'seed':int(details[1]), 
                    'ini':int(details[2]), 'model':model, 'epochs':int(details[3]),
    ...
    for f in glob.glob(loc+"/Experiments/IO/%s/*.pt" %s):
        ...
        if (len(details) == 5) & (details[0][:5] == "PINNO"):
            ...
            multiplier = 3
            newwidth = int((-(sys.netshape1*multiplier+3)+np.sqrt((sys.netshape1*multiplier+3)**2 - 4*1*(sys.netshape1*2 - (sys.netshape1*sys.netshape2 + sys.netshape2*sys.netshape2 + sys.netshape2 + sys.netshape1 + sys.netshape2*2))))/(2))
            net = SnakeParamNet(sys.netshape1,newwidth,1)
            net.load_state_dict(torch.load(f))
            model = "pHNN-IO"
            net = net.to(device)
            data = pd.Series({'seed':int(details[1]), 
...
plot_order = ["HNN", "pHNN-O", "pHNN-L", "pHNN-I", "pHNN-OL", "pHNN-LI", "pHNN-IO", "pHNN-OLI"] #["HNN", "pHNN-O", "pHNN-L", "pHNN-I", "pHNN-OL", "pHNN-LI", "pHNN-OI", "pHNN-OLI"]
...
f = sns.barplot(data=totaldf[totaldf["Experiment"].isin(["Non-linear Pendulum","System1","System2","System3","System4","Double Pendulum"])], x="Experiment", y="Vector Error", hue="model", errorbar = "se", 
		palette = sns.color_palette("mako"), hue_order = plot_order,
                order = ["Non-linear Pendulum","System1","System2","System3","System4","Double Pendulum"],
                ax=axes[1]); f.set_ylabel(r'$E_{V}$', rotation=90, labelpad=30); f.set_xlabel(None); axes[1].yaxis.set_major_formatter(mtick.PercentFormatter()); axes[1].yaxis.set_minor_formatter(mtick.FormatStrFormatter("%.2f")); f.legend_.remove();
...
axes[0].plot(4.65, 20000, "*", markersize=20, color="r")
axes[1].plot(4.65, 1500, "*", markersize=20, color="r")
...
plt.savefig("Experiment10_5L3error.pdf", format="pdf", bbox_extra_artists=(lgd,), bbox_inches='tight') 
plt.clf()
...
"""
    ...
    print("%s & %.2E (%.2E) & %.2E (%.2E) & %.2E (%.2E) & %.2E (%.2E)" %(s, df[df["model"]=="HNN"]["loss_mean"], df[df["model"]=="HNN"]["loss_sem"], 
								df[df["model"]=="spHNN-O"]["loss_mean"], df[df["model"]=="spHNN-O"]["loss_sem"],
								df[df["model"]=="spHNN-L"]["loss_mean"], df[df["model"]=="spHNN-L"]["loss_sem"],
								df[df["model"]=="spHNN-I"]["loss_mean"], df[df["model"]=="spHNN-I"]["loss_sem"],))
    print("%s & 0.00 \\%% & %.2f \\%% & %.2f \\%% & %.2f \\%% " %(s, (1-df[df["model"]=="spHNN-O"]["loss_mean"].values/df[df["model"]=="HNN"]["loss_mean"].values)*100, 
								(1-df[df["model"]=="spHNN-L"]["loss_mean"].values/df[df["model"]=="HNN"]["loss_mean"].values)*100, 
								(1-df[df["model"]=="spHNN-I"]["loss_mean"].values/df[df["model"]=="HNN"]["loss_mean"].values)*100,))
    print("%s & %.2E (%.2E) & %.2E (%.2E) & %.2E (%.2E) & %.2E (%.2E)" %(s, df[df["model"]=="HNN"]["Hloss_mean"], df[df["model"]=="HNN"]["Hloss_sem"], 
								df[df["model"]=="spHNN-O"]["Hloss_mean"], df[df["model"]=="spHNN-O"]["Hloss_sem"],
								df[df["model"]=="spHNN-L"]["Hloss_mean"], df[df["model"]=="spHNN-L"]["Hloss_sem"],
								df[df["model"]=="spHNN-I"]["Hloss_mean"], df[df["model"]=="spHNN-I"]["Hloss_sem"],))
    print("%s & 0.00 \\%% & %.2f \\%% & %.2f \\%% & %.2f \\%% " %(s, (1-df[df["model"]=="spHNN-O"]["Hloss_mean"].values/df[df["model"]=="HNN"]["Hloss_mean"].values)*100, 
								(1-df[df["model"]=="spHNN-L"]["Hloss_mean"].values/df[df["model"]=="HNN"]["Hloss_mean"].values)*100, 
								(1-df[df["model"]=="spHNN-I"]["Hloss_mean"].values/df[df["model"]=="HNN"]["Hloss_mean"].values)*100,))
	
    print("%s & %.2f (%s) & %.2f (%s) & %.2f (%s) & %.2f (%s)" %(s, df[df["model"]=="HNN"]["time_mean"].values, df[df["model"]=="HNN"]["epochs_mean"].values[0], 
								df[df["model"]=="spHNN-O"]["time_mean"].values, df[df["model"]=="spHNN-O"]["epochs_mean"].values[0],
								df[df["model"]=="spHNN-L"]["time_mean"].values, df[df["model"]=="spHNN-L"]["epochs_mean"].values[0],
								df[df["model"]=="spHNN-I"]["time_mean"].values, df[df["model"]=="spHNN-I"]["epochs_mean"].values[0],))
"""
